...
function Software() {
    ...
    const getCategories = async () => {
        let result = await Fetch(`${host}/api/categories?service=software_developer`, 'GET');
    ...
    return (
        <Box>
            ...
            <Box className="w-full h-screen pt-32">
                <Typography variant="h2" fontWeight={800} className="text-white text-center !my-5">تطوير البررمجيات</Typography>
                <Typography variant="h3" className="text-white text-center !mt-5">حول فكرتك إلى حل رقمي قوي قابل للتوسع</Typography>
            </Box>
            ...
            <Box id="request" sx={{ right: language === 'en' && '0' }} className="w-screen h-screen fixed top-0 left-1/2 -translate-x-1/2 bg-gray-200 bg-opacity-5 hidden justify-center items-center">
                <RequestNewService service='software_developer' setSnackBar={setSnackBar} onClickCancel={() => setPopup('request', 'none')} />
            </Box>
