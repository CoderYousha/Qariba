                    ...
                    <Box sx={{ backgroundColor: theme.palette.background.default }}>
                        <Box className="w-full pt-10 rounded-xl relative" dir={language === 'en' ? 'ltr' : "rtl"} sx={{ float: language === 'en' && 'right' }}>
                            <Box className='w-3/4 mx-auto px-10 py-10 rounded-lg' sx={{ backgroundColor: theme.palette.background.paper }}>
                                {
                                    profile.image &&
                                    <img className="w-24 h-24 rounded-full mx-auto mb-5" src={`${host}/${profile.image}`} />
                                }
                                <TextField variant="outlined" label='الاسم الكامل' value={fullNamee} onChange={(e) => setFullName(e.target.value)} className="w-full" />
                                <TextField variant="outlined" label='رقم الهاتف' value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full !mt-10" />
                                <TextField variant="outlined" label='البريد الإلكتروني' value={email} aria-readonly className="w-full !mt-10" />
                                ...
                                <Box className='mt-10 text-center'>
                                    ...
                                    <Box className='mt-7 text-center'>
                                        <Typography onClick={() => setPopup('update_password', 'flex')} variant="body1" className="cursor-pointer w-fit !mx-auto">تعديل كلمة المرور</Typography>
                                    </Box>
                                </Box>
                            </Box>
                        </Box>
